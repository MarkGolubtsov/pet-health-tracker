...
Файл содержит одну строку в формате `version=<value>`.
Формат значения: `MAJOR.MINOR.PATCH` или `MAJOR.MINOR.PATCH-SNAPSHOT`.
Суффикс `-SNAPSHOT` используется для незарелизенного состояния проекта.
Текущая версия: `0.0.0-SNAPSHOT`.
Compose передает `version.txt` в runtime-окружение сервисов.
В контейнерах значение доступно как переменная окружения `version`.
